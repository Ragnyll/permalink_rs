...
fn main() {
    ...
    git_commit_link_scheme.insert(
        String::from("git@gitlab.com"),
        String::from("https://gitlab.com/REPO-NAME/-/blob/COMMIT/FILE#LINE_RANGE"),
    );
    git_commit_link_scheme.insert(
        String::from("git@github.com"),
        String::from("https://github.com/REPO-NAME/blob/COMMIT/FILE#LINE_RANGE"),
    );
    ...
    let matches = App::new("git-permalink-rs")
        ...
        .arg(
            Arg::new(PATH_PARAM)
                .required(true)
                .takes_value(true)
                .about("the path of the file to permalink"),
        )
        .arg(
            Arg::new(LINE_RANGE_PARAM)
                .required(true)
                .takes_value(true)
                .about("the line or lines to permalink. takes format startLine[-endLine]"),
        )
        .get_matches();
    ...
    if path.exists() {
        ...
        env::set_current_dir(parent_dir).expect(&format!(
    ...
    let mut url_pattern = "";
    let stdout =
        String::from_utf8(output.stdout.to_vec()).expect("Unable to parse stdout as valid Utf8");
    ...
    let mut lines: Vec<&str> = matches
        .value_of(LINE_RANGE_PARAM)
        .unwrap()
        .split("-")
        .collect();
    ...
    let git_repo_base_path =
        String::from_utf8(output.stdout.to_vec()).expect("Unable to parse stdout as valid Utf8");

    let relative_path_to_file =
        pathdiff::diff_paths(Path::new(&path), Path::new(&git_repo_base_path)).unwrap();
    // for some reason there is always an unnessecary ../ just drop it.
    let relative_path_to_file =
        String::from(relative_path_to_file.into_os_string().to_str().unwrap()).replace("../", "");
    let relative_path_to_file = relative_path_to_file
        .split("/")
        .collect::<Vec<&str>>()
        .into_iter()
        .skip(1)
    ...
    let final_link = url_pattern
        .replace("REPO-NAME", &repo_name)
        .replace("COMMIT", &current_commit)
        .replace("FILE", &relative_path_to_file)
